...
```js
event$.useSubscription('hello', val => {
  console.log(val)
})
```
...
| 参数            | 说明             | 类型                                                   |
| --------------- | ---------------- | ------------------------------------------------------ |
| emit            | 发送一个事件通知 | `(key: React.Key, val: T) => void`                     |
| off             | 取消订阅事件     | `(key: React.Key) => void`                             |
| useSubscription | 订阅事件         | `(key: React.Key, callback: (val: T) => void) => void` |
